...
public class GeometryServlet extends HttpServlet {
    ...
    // Геометрическая фигура фестиваля, является массивом координат на карте в формате JSON
...
//            "            ]";
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        Festival[] festivalsArray = getCurrentFestivals();

        Gson gson = new Gson();
        String message = gson.toJson(festivalsArray);
    ...
    private Festival[] getCurrentFestivals(){

        List<Festival> festivals= FestivalServiceImpl.getInstance().getAllList();

        int countOfFestivals = festivals.size();
        System.out.println("festivals count:" + countOfFestivals);

        Festival[] festivalsArray = new Festival[countOfFestivals];

        festivals.toArray(festivalsArray);
